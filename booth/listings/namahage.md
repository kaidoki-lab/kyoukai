# なまはげ OBS素材パック ── 泣く子はいねぇか

## 商品説明文

なまはげの部屋では、本体画像に低解像度のドット目Canvasが重なり、赤く光る瞳がこちらを見つめている。目が明滅を繰り返し、時折「──いるか」の一言が画面に浮かぶ。逃げる場所はない。

【同梱内容】3点

  01_waiting / waiting.html
    待機画面。観測 中の状態を表示し続ける。
    配信前、ゲームロード中、場面転換に使用する。

  02_brb / brb.html
    離席中画面。「なまはげを呼んでいます」というメッセージが表示される。
    離席時、準備中、一時中断時に使用する。

  03_lower-third / lower_third.html
    ローワーサード（名前テロップ）。透過背景。
    他の素材の上に重ねてオーバーレイとして使用する。

【OBS設定方法】

  1. OBS Studio を開く
  2. ソース → ＋ → ブラウザ を追加
  3. 「ローカルファイル」にチェックを入れる
  4. 各 .html ファイルを指定する
  5. 幅: 1920  /  高さ: 1080 に設定する

  ローワーサードの名前・肩書きは lower_third.html 内の該当箇所を
  書き換えることで変更できる。

【動作環境】

  OBS Studio 29以降推奨
  インターネット接続不要 / 完全ローカル動作

【利用規約】

  個人・商用配信どちらでも使用可
  再配布・転売 禁止
  改変は個人使用の範囲で自由

KYOUKAI ─ 境界
https://kyoukai.vercel.app

## タグ

OBS / 配信 / オーバーレイ / 待機画面 / 配信素材 / 民俗系 / 瞳演出 / KYOUKAI / 不穏系配信 / ホラー系配信

## 価格

300円
