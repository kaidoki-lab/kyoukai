# 粒子観測 OBS素材パック ── 意味より先に運動がある

## 商品説明文

粒子観測は、無数の粒子群を見つめる部屋。十字の観測レティクルが最も近い粒子を追い、座標を表示し続ける。衝突を避け、方向を変え、群れていく粒子たちに、明確な目的は与えられていない。

【同梱内容】3点

  01_waiting / waiting.html
    待機画面。粒子 観測中の状態を表示し続ける。
    配信前、ゲームロード中、場面転換に使用する。

  02_brb / brb.html
    離席中画面。「粒子を観測しています」というメッセージが表示される。
    離席時、準備中、一時中断時に使用する。
...
OBS / 配信 / オーバーレイ / 待機画面 / 配信素材 / パーティクル / 粒子演出 / KYOUKAI / 不穏系配信 / 観測系
