# 評議録 OBS素材パック ── 発言者は不明のまま

## 商品説明文

評議録には、和紙質感の画面に縦書きの議事断片が浮かんでは消えていく。発言者は不明。議題も結論も曖昧なまま、毛筆風の名札だけが記録されていく。合意しているのか、対立しているのかは判別できない。

【同梱内容】3点

  01_waiting / waiting.html
    待機画面。記録 進行中の状態を表示し続ける。
    配信前、ゲームロード中、場面転換に使用する。

  02_brb / brb.html
    離席中画面。「議事を再開します」というメッセージが表示される。
    離席時、準備中、一時中断時に使用する。
...
OBS / 配信 / オーバーレイ / 待機画面 / 配信素材 / 議事録風 / タイプライター / KYOUKAI / 不穏系配信 / テキスト演出
