...
WORLDVIEW = {
    'observation': (
        '観測域は、KYOUKAIの中でもっとも活動が多い部屋。画面全体を流れる観測ログ端末に、生命体の記録が絶えず積み上がっていく。'
        'ログは感情を持たない。だが、それを見ていると何かを感じる。観測対象が誰なのかは、最後まで語られない。'
    ),
    'hyougi': (
        '評議録には、和紙質感の画面に縦書きの議事断片が浮かんでは消えていく。発言者は不明。議題も結論も曖昧なまま、'
        '毛筆風の名札だけが記録されていく。合意しているのか、対立しているのかは判別できない。'
    ),
    'null': (
        '崩落域は、画面自体が傾き、UI部品がずり落ちていく部屋。データはある。だが何のデータかはわからない。'
        'ページとして機能しているが、周期的に崩壊と再構築を繰り返す。長く留まると、自分が何をしていたのかも曖昧になる。'
    ),
    'observer': (
        '逆観測室は、KYOUKAIの最深部。中央の瞳孔のような円環が、こちらをゆっくりと見つめ続ける。'
        '静かに、そっと、こう告げられる。あなたはずっと観測されていた。'
    ),
    'exit': (
        '境界域は、進んでは戻るロード画面そのものの部屋。プログレスバーが進捗を示すが、決して満ちることはない。'
        '進んでいるつもりでも、見ているのは境界を越えようとしていた誰かの断片。終わりのようで、終わりではない。'
    ),
    'archive': (
        '記録室には、ファイル棚のグリッドに感情を排した観測記録が積み上がっている。日付、ID、内容だけのカードが1枚ずつめくれる。'
        '感情は排除されているはずなのに、読むと何かが生まれる。削除されたはずの記録が、まだそこに残っている気配がある。'
    ),
    'ma': (
        '悪魔の間は、逆観測室の奥にある隠し部屋。呼吸するように明滅する赤い光だけが、計器も装飾もなく画面を支配する。'
        '最初は沈黙するだけだが、何度も訪れるうちに、光の呼吸のあいだに短いセリフが差し込まれ始める。'
    ),
    'daimyojin': (
        'AI大明神は、信仰と機械が混ざり合った祈願装置。神社と端末が同じ本体画像の中に同居し、祈願札が発光しながら揺れる。'
        '占いのように見えるが、処理されているのが願いなのか観測結果なのかは、最後まで明かされない。'
    ),
    'gokuraku': (
        '極楽域は、CSSグリッドで描かれた引き出し棚が積み上がった奥の部屋。「極楽」という言葉は、ここでは明るく響かない。'
        '引き出しが1つずつ開閉し、埋め込まれた音量バーの音源番号だけが静かに変わっていく。'
    ),
    'particles': (
        '粒子観測は、無数の粒子群を見つめる部屋。十字の観測レティクルが最も近い粒子を追い、座標を表示し続ける。'
        '衝突を避け、方向を変え、群れていく粒子たちに、明確な目的は与えられていない。'
    ),
    'ripple': (
        '波紋域は、本体と同じドットグリッドに触れることで世界が応答する部屋。暗赤から赤、黄、青、白へとドットが色を変えていくが、'
        '一定周期で波紋が中心から広がり、画面全体を黒く初期化する瞬間が訪れる。'
    ),
    'kanrinin': (
        '管理人室は、KYOUKAIの管理施設。受付カウンターの本体画像の上で、呼び鈴と鍵ボックスが発光ポイントとして点在する。'
        '「席を外しています」の札が揺れ、時折目玉がうっすら透けて見える。管理人本人の姿は、決して見えない。'
    ),
    'namahage': (
        'なまはげの部屋では、本体画像に低解像度のドット目Canvasが重なり、赤く光る瞳がこちらを見つめている。'
        '目が明滅を繰り返し、時折「──いるか」の一言が画面に浮かぶ。逃げる場所はない。'
    ),
    'matsuri': (
        '棒入れ祭は、本体の棒・穴・紙吹雪素材を配置した豊穣信仰の奉納儀式画面。奉納カウンタが数字を刻み、'
        '打ち上げ花火のようなスパークが上がり続ける。人物は誰も描かれていない。'
    ),
    'fukashitsu': (
        '卵部屋では、ピンク色の孵化装置の本体画像の中で、卵部分の粒子がゆっくりと明滅し色を帯びていく。'
        '栄養、酸素、温度。三つの計器パステルカラーの奥で、卵の鼓動だけが静かに続いている。'
    ),
    'typhoon-news': (
        '台風ニュースは、本体のニュース背景画像を使った速報室。速報帯と台風情報パネルが本体同様のUI構成で流れ続けるが、'
        '台風の名前だけが「後ほど連絡します」「お母さんに聞いて」といった、日常の保留語に置き換わっている。'
        '離席画面だけは全部屋で唯一、放送中断を示す砂嵐が流れる。'
    ),
}
