...
| 境界域 | /exit | 現在の街エリア。街を歩きながら建物を探索し、それぞれの部屋へ入れる |
| 記録室 | /archive | よく分からない画集を閲覧できる部屋 |
| 外部接続 | /outside | KYOUKAIの「外」への扉。世界の外縁 |
| 悪魔の間 | /ma | 逆観測室の隠し扉の先。再訪によって大魔将との関係が変化する |
| AI大明神 | /daimyojin | AIから神託が届くが、内容はまったく意味がない |
...
- 投稿候補上の見どころ: AIから神託が届くが、内容はまったく意味がない
...
`data/rooms/{room_id}.md` の `投稿候補情報` と、ShortFACTORY の `post_candidates` 生成で使う見どころ・投稿文候補を以下で確定した。
...
KYOUKAIの通常シナリオとして、Route_C「壊れる前の形」の次に発生する Route_D「集まっているもの」を追加した。

Route_Dは、6Fの5部屋を使って、KYOUKAI内に散らばっていた反応が「声」や「形」になりかける様子を見せるルートである。ただし、このルートでは声は成立しない。

開始条件は以下。

- シナリオモードであること。
- Route_A、Route_B、Route_C が completed であること。
- Route_D が not_started であること。
- active_route_id が null であること。
- `floor_06` が開放済みであること。
- Route_C 管理人帰還イベント完了後、一度管理人室を退出し、再入室していること。
- 管理人室へ再入室後、20秒経過していること。
- 他の電話イベントが発生していないこと。
...
1. `route_d_phone_001`
2. `route_d_room_ripple_001`
3. `route_d_room_colony_001`
4. `route_d_room_dot_art_001`
5. `route_d_room_matsuri_001`
6. `route_d_room_namahage_001`
7. `route_d_manager_return_001`

使用部屋は以下。

- 共通ハブ: `kanrinin`
- Route_D 専用主要部屋: `ripple`, `colony`, `dot-art`, `matsuri`, `namahage`

Route_D 完了時の状態変化は以下。

- Route_D を completed にする。
- active_route_id を null に戻す。
- completed_scenario_count を +1 する。
- `ripple`, `colony`, `dot-art`, `matsuri`, `namahage` は `post_route_d` 状態を保持する。
- current_target_room_id を null に戻す。

Route_D 完了では新規階層を開放しない。最終シナリオ、最上階、消滅の鍵、巨大鍵穴、最上階鍵穴も開放しない。
...
Route_D以降の進行は、単純な階層開放ではなく「何に触れたか」を条件にする方針とする。管理人会話の最後の要点は「触ったものが、次の形になります」。
...
- `static/kyoukai-scenario-events.js`
- `static/kyoukai-scenario.js`
- `static/kyoukai-route-d-room.js`
- `static/kyoukai-scenario-ui.css`
- `templates/ripple.html`
- `templates/colony.html`
- `templates/dot-art.html`
- `templates/matsuri.html`
- `templates/namahage.html`
