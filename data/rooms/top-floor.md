...
建物データでは `topFloorRoom` として通常の `rooms` 配列とは分離して管理する。`/top-floor` は室内画像を全画面寄りに表示し、エレベーターへ戻る導線と、Route_E終盤でのみ反応する鍵穴ホットスポットを持つ。

Route_Eの最終電話完了後、`top_floor_unlocked === true` かつ `active_route_id === "route_e"` の時だけ入室・鍵穴操作が有効になる。初回入室では `top_floor_entered` と `top_floor_entered_at` を保存し、鍵穴は `keyhole_state` で `inactive / available / waiting_for_key / ready / processing / completed` を管理する。

制作物03時点では、鍵なし接触時の短い反応と、消滅の鍵を持っている場合に制作物04へ渡す接続フックまでを実装する。消滅の鍵の取得、使用演出、逆観測室への本遷移、Route_E完了処理は後続仕様で確定する。
