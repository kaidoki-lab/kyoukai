...
### 未確認接続（/null）
エラーでも終わりでもない。繋がっているが定義できない状態。
- データがある。でも何のデータかわからない
- ページとして機能しているが、何のためかは不明
- 崩落と静寂が同時にある
...
### 境界域（未確定）
KYOUKAIプロジェクト全体の核。
- 具体的なルートを持たない。あらゆる部屋の「間」にある
- 実装するのではなく、感じさせる概念
- 全ての部屋が境界域に向かって収束している設計意図がある
- 言語化すると消える。定義を拒むこと自体が機能
