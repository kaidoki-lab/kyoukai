...
**完了条件**:
- [x] 4部屋×3ファイルが専用実装（legacy委譲なし。台風ニュースbrbの砂嵐は専用実装として書き直した上での採用）
- [x] verify_packs.py 対象12ファイルpass
- [x] diff_check.py 対象12ファイル差分あり
- [x] legacy_templates.py への参照が room_specs 全16ファイルから消えている（grepで確認）
...
**完了条件**:
- [x] verify_packs.py 拡張版で48ファイル全pass(JSエラー0+差別化チェックpass)
- [x] zip17本・サムネイル64枚(全て1920x1080)・listings19ファイルが新デザイン基準で更新済み
- [x] final_check.py ALL PASS
- [x] コミット完了(親セッションでコミット)
